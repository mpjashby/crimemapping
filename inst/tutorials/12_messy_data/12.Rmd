...
burglary_values <- c(1320, 653, 2068, -1, 580)
...
natural features to make up a base map, and buildings on other features (such as
...
problem is if several of the columns form a group, separate from the others. In
this case, there is a group of columns that show crime counts that is different
from the other column that shows the area name.
...
learn how to deal with this issue later in a future tutorial.
...
so row 34 on the spreadsheet is actually row 30 in our data. Knowing this, we 
can remove all the rows below row 30 using `slice()`:
...
```{r structure-exercise8, exercise=TRUE, exercise.setup="victims-table"}
...
```{r structure-exercise9, exercise=TRUE, exercise.setup="victims-table"}
separate(
  victims, 
  col = age_sex, 
  into = c("age", "sex"), 
...
automatically when we load `tidyverse`.
...
uk_data %>% 
  mutate(location = str_remove(location, "On or near "))
```
...
<img src="images/parse_number.jpg" alt="Cartoon showing that the parse_number() function extracts numeric values from text, stripping out all non-numeric characters." class="right-side-image">
...
uk_data %>% 
  mutate(location = recode(
    # Once again we will remove the unnecessary 'On or near '
...
# Print some burglary values, stored as a vector
burglary_values

# Try to calculate the mean value -- no error, but the answer is wrong
mean(burglary_values)

# Convert `-1` to `NA` first, now you get the correct mean value
burglary_values <- ifelse(burglary_values == -1, NA, burglary_values)
mean(burglary_values, na.rm = TRUE)
```
...
`addresses` object that matches the formating changes we made to the original
address.
...
with. You can find out more about data cleaning and tidying with these 
resources:

...
# Map data
# ****** REPLACE THIS WITH SOMETHING BASED ON `group_modify()` ******
...
later). **Avoid using monthly counts of crime in almost all circumstances**.
...
three times to create the KDE layers for each shift, but this is a good 
situation in which to use the `map()` function from the `purrr` 
package to run a piece of code repeatedly for several separate pieces of data.

To prepare the data to be used by `map()`, we first have to *nest* the data.
Nesting data means that we collect up bunches of rows from our dataset and store
each bunch as a tibble stored _inside_ a column in a dataset. In a nested 
dataset, each row will contain one or more columns that themselves contain a
whole sub-dataset. 

We create a nested dataset using the `nest()` function from the `tidyr` package 
(loaded automatically by `tidyverse`). `nest()` is easier to work with if our
dataset contains only the columns we need, so before we use `nest()` we'll use
`select()` to remove all the columns that we don't need. We can then specify
which columns in the dataset we want to be nested and what we want the new nest
column to be called. We do this in the form:

```r
nest(column_name = c(vector_of_columns, that_we_want, to_nest))
```

In this case, we only want to nest the `geometry` column in the data and we want
the nest column to be called `data`, so we use `nest(data = geometry)`. Let's
look at the first few rows of the nested data:

```{r map-exercise1a, exercise=TRUE}
nested_assaults <- assaults_by_shift %>% 
  # Remove all the columns except the ones we want
  select(shift, geometry) %>% 
  # Nest the `geometry` column
...
head(nested_assaults)
```

You can see that the nested dataset has two columns: `shift` and `data`. The
`data` column contains all the rows of `geometry` corresponding to each value of
`shift`. The `shift` column exists because we did not specify that it should be
nested inside the `data` column – `nest()` will use any columns we don't specify
should be nested to differentiate between the different groups of nested data. 
In this case, this means we can use the `shift` column at a later step.

We can look inside the nested column using the `pluck()` function from the 
`purrr` package, which we have used in previous tutorials. For example, by using
`pluck(nested_assaults, "data", 1)` we can extract and view the first row in the
`data` column in the `nested_assaults` object.

```{r map-exercise1b, exercise=TRUE}
pluck(nested_assaults, "data", 1)
```

From this, we can see that the first row in the `data` column of the
`nested_assaults` object contains an SF object with one column (`geometry`) and
`r scales::comma(nrow(pluck(nested_assaults, "data", 1)))` rows. 

Now that we've nested the data, we can use `map()` to apply the `hotspot_kde()`
function to the data stored in each row of the the nested column in our dataset.

```{r map-exercise2, exercise=TRUE, exercise.lines=21}
library(sfhotspot)

kde_layers <- assaults_by_shift %>% 
  # Remove all the columns except `shift` and `geometry`
  select(shift, geometry) %>% 
  # Bunch the `geometry` column of the data into three datasets, one for each 
  # shift
  nest(data = geometry) %>% 
  ...
  # `bind_rows()`. Remember that the first argument to `map()` is the function
  # we want to apply to each group of data, while any remaining arguments are
  # passed to the function we're using. This means we can pass arguments like
  # `cell_size` to `hotspot_kde()` by providing them as arguments to `map()`.
  deframe() %>% 
  map(hotspot_kde, cell_size = 250, bandwidth = 1000) %>% 
  bind_rows(.id = "shift") %>% 
  st_transform(4326)
...
<h5 id="map-box2-title" class="box-title">What do `deframe()` and `bind_rows()` do?</h5>

<div id="map-box2" class="box-content">

`map()` works on a _list_ of objects (in this case, the nested SF objects).
`deframe()` (from the `tibble` package, which is loaded by `tidyverse`) converts 
our dataset of nested SF objects into a list of SF objects so that `map()` 
can use it. More exactly, `deframe()` converts any dataset with two columns into 
a list of objects, converting the first column of the dataset to the names of 
the list elements and the second column to the values of the list elements. So 
if we had a nested dataset that looked like this (note that each row in 
`second_column` contains a nested tibble):

```{r}
tribble(
  ~first_column, ~second_column,
  "a_name", tibble(a = 1, b = 2, c = 3),
  "another_name", tibble(a = 4, b = 5, c = 6),
  "yet_another_name", tibble(a = 7, b = 8, c = 9)
)
```

We could use `deframe()` to convert it into a list that looked like this:
...
`map()` produces a list, with each element in the list containing the results of
applying the function – in our case, `hotspot_kde()` – to each dataset. Since
we want a single SF object rather than a list of them, we can use the 
`bind_rows()` function from `dplyr` to bind the results into a single SF object.

It's important that `deframe()` produces a _named_ list because it means we can
use the `.id` argument to `bind_rows()` to specify that the names of the list 
items should be included in a new column in the result returned by `map()`. This
converts the list above so that it's a single dataset again:
...
tribble(
  ...
  "another_name", tibble(a = 4, b = 5, c = 6),
  "yet_another_name", tibble(a = 7, b = 8, c = 9)
) %>% deframe() %>% map(identity) %>% bind_rows(.id = "name")
```

This approach to applying a function separately to different parts of a dataset
is called _split, apply, combine_ and it is a very common way of wrangling data
...
`map()` to avoid having to create 24 different objects and then binding them
together.
...
<h5 id="animated-box1-title" class="box-title">What does `deframe()` do?</h5>

...
# Load data
frauds <- read_csv("../../extdata/kansas_city_frauds.csv.gz")
```
...
clicks the wrong button in other pieces of software. 
...
print the object (by typing the name `frauds` into the R console) or use the 
`names()` function to print a list of column names present in the data:
...
```{r debugging-cartoon, fig.align="center", out.width="80%"}
...
that you should be aware of, R has three ways of communicating with you. The
first is by printing a *message* in the R console. Messages are usually for 
information only and typically don't require you to take any action. For example,
the function `get_crime_data()` from the `crimedata` package issues a message
to tell you what data it is downloading because this sometimes takes a few 
seconds and without a message you might wonder if the code was working.
...
used in a function to track down the error. We will look at this in the next
section.
...
```{r man-page, fig.align="center", out.width="80%"}
knitr::include_graphics("images/man_page.png")
```
...
All manual pages have the same format.
  ...
  * **Value** explains the type of value that the function will return, and 
    ...
    was `NA` then mean will return `NA` *unless* the argument `na.rm = TRUE` is
...
`there is no package called …` means you've either mis-typed a package name or that package is not installed

`could not find function …` means you've either mis-typed a function name or not loaded the relevant package

`object not found …` usually means you've mis-typed an object name
-->
...
<img src="images/code_hero.jpg" style="width: 50%; max-width: 500px; float: right; margin: 0 0 2em 2em;">
...
```{r make-deaths-data, eval=FALSE}
# THIS CODE IS NOT RUN BUT IS KEPT HERE FOR REFERENCE
Seatbelts %>% 
  as_tibble() %>% 
  mutate(
  ...
  write_csv("../../extdata/road_deaths_data.csv")
```

```{r reprex-exercise1-setup}
# this code downloads the data and stores it in the temporary directory created
...
read_csv("https://github.com/mpjashby/crimemapping/raw/main/inst/extdata/road_deaths_data.csv") %>% write_csv("road_deaths_data.csv")
```
...
the same error. To start with, it is possible to remove all the lines of code
that fine-tune the appearance of the chart (lines 23--38 above). We can also
remove the lines creating the trend line (line 22) and still get the same error, 
as well as the code that wrangles our data in ways that don't affect the error 
(lines 9--15). Finally, we now no-longer need to load the `dplyr` or `magrittr` 
packages that provide data-wrangling functions and the `%>%` operator, because 
...
If the code were to run successfully then the resulting chart would look quite
different to the original chart we wanted, but that does not matter because what
we are interested in is showing code that produces a specific error. If you
run this code, you will see it produces the same error as the code above.
...
variables are in the `mpg` dataset by just typing the dataset name into the R
console.
...
We have now managed to reduce our original 37 lines of code down to 9 lines, as
...
```{r reprex-package, fig.align="center", out.width="80%"}
knitr::include_graphics("images/reprex-cartoon.jpg")
```
...
that community is probably the website [Stack Overflow](https://stackoverflow.com/).
This is a website for people who are writing code in any programming language
imaginable to get help. It is part of the larger [Stack Exchange Network](https://stackexchange.com)
of question-and-answer websites covering everything from [travel](https://travel.stackexchange.com) 
to [veganism](https://vegetarianism.stackexchange.com/).

To ask a new question on Stack Overflow, go to [stackoverflow.com/questions/ask](https://stackoverflow.com/questions/ask) and
...
your question. Questions are more likely to get an answer faster if you:
  ...
  * Give the question a specific title. Over 20 million questions have been 
    ...
    question. [Look at some recent questions about R on Stack Overflow](https://stackoverflow.com/questions/tagged/r) to get some ideas on
    what title to give for your question.
...
Common questions are typically answered within a few hours. Hopefully this will
final stage of the emotional roller coaster of debugging:

```{r debugging-cartoon2, fig.align="center", out.width="80%"}
knitr::include_graphics("images/debugging.jpg")
```
...
If you replace `+` with `%>%` on line 7 of the code below, the code should now

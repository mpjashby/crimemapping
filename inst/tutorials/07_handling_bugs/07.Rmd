...
time until you find the one that causes the problem. 
...
the line `filter(offense_code == "26G")` results in zero rows of data -- all the 
rows have been removed by the `filter()` function. This is because `26G` is not 
a valid offence code in this dataset. This means that no rows are kept in the 
dataset by `filter()` because there are no rows for which 
`offense_code == "26G"`. In turn, this means there are no rows for 
...
common_errors <- tribble(
  ...
  "`The pipe operator requires` `a function call as RHS`", "There is a pipe operator at the end of the final line of a code pipeline (or what R thinks should be the final line)."
)

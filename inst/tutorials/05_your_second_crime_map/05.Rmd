...
As you saw in the video, generally vector data gives you more control over the
...
few rows of the `bike_thefts` dataset.
...
random_points_kde <- random_points |> 
  hotspot_kde(grid = random_points_grid) |> 
  mutate(
...
makes reasonable decisions about how or density maps should look, while still 
giving us control over their appearance if we want it. `sfhotspot` also has 
other useful functions that we will use in future tutorials.
...
meaningful, so we can ignore these for now.
...
detail in your map by using a smaller bandwidth, you could use `adjust = 0.75` 
or `adjust = 3/4` to set the bandwidth to be three-quarters of the default 
bandwidth.
...
bike_theft_hull <- bike_thefts_sample |> 
  ...
  st_union() |> 
  st_convex_hull()
bike_theft_sample_kde <- hotspot_kde(bike_thefts_sample, bandwidth_adjust = 0.5)
...
zoom_map <- map(8:16, function(zoom_level) {
  ggplot() + 
    annotation_map_tile(zoom = zoom_level) +
...
basemaps_map <- c(
  ...
  map(function (x) {
    ggplot() +
      annotation_map_tile(type = x, zoom = 12) +
      geom_sf(data = jdi, colour = NA, fill = NA) +

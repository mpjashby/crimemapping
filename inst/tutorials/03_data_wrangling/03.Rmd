...
description: "Learn the building blocks of data analysis: loading, filtering, manipulating and saving data for crime mapping."
output: 
  learnr::tutorial:
    # progressive: true
...
One basic R function is `sqrt()`, which calculates the square root of a number.
The `sqrt()` function has only one argument: the number that we want to find the 
square root of. 


::: {.tutorial}

Type `sqrt(2)` in the box below and click `Run Code` to calculate the square 
root of 2.
...
case, just the number ``r format(sqrt(2), digits = 7)`` (for now, you can ignore 
the number `[1]` in square brackets).
...
be tedious, so in RStudio we can use the keyboard short cut `Option+-` (on Mac) 
or `Alt+-` (on Windows or Linux) to insert the complete operator.
...
We can use `read_csv()` to load data from `https://mpjashby.github.io/crimemappingdata/san_francisco_robbery.csv` 
and store it in an object called `san_fran_rob`. 


::: {.tutorial}

Use the code above to help you, or click the `Solution` button to reveal the 
answer.
...
Type the code needed to view the first few rows of the `san_fran_rob` object and 
...
Write the code necessary to select the `longitude` and `latitude` columns from 
the `agg_assault_data` object:
...
offences in which the `location_category` is 'retail' and the `location_type`
...
we then get a summary for each group separately. We can use the `.by` argument
of the `summarise()` function to specify that we want separate summaries for
each unique value of one or more columns in the data. For example, to produce a
separate summary for each unique value of `location_category`, we can use this
code:
...
You can add multiple grouping variables using the `c()` (_combine_) function if 
...
crimes of each type are in the dataset. We can use `summarise()` to do that,
together with the `n()` function (from the same `dplyr` package as 
`summarise()`). For example, if we wanted to count how many rows in the
`agg_assault_data` dataset had each unique combination of `location_category` 
and `location_type`:
...
In this code, the `n()` function simply returns the number of rows of data in
each group, i.e. the number of rows with each unique combination of values of
`location_category` and `location_type`.

Since counting the number of rows in each group a dataset is a very common task,
`dplyr` includes another function called `count()` that allows you to do the
same thing as in the code above, but with slightly less typing:

So if you wanted to know how many aggravated assaults had occurred in each 
location category and type:

```{r summarise-exercise5, exercise=TRUE}
...
that the reference to the column `weekday` on line 12 of this code belongs to 
the `count()` function on line 1.
...
`x  |> fun1() |> fun2(y)` is the same as the code `fun2(fun1(x), y)`, but it is 
much easier to see that `fun1()` is run before `fun2()`. It may be useful to 

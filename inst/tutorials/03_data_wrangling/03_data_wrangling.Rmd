...
A major step in using any data to make decisions or draw conclusions in data
...
knitr::include_graphics("images/datawrangling.jpg")
```
...
it. The code to use a function 
(sometimes called *calling* the function) has two parts: the function name 
followed by a pair of parentheses, inside which are zero or more *arguments* 
separated by commas. Arguments are a way of providing input that a function 
works on, or to fine-tune the way the function works (we will see many examples
of this later).
...
on. Unfortunately, this shortcut only works in the R console, not in this 
tutorial.
...
<a href="https://tidyverse.org/" title="tidyverse website"><img src="images/tidyverse.png" style="width: 33%; max-width: 150px; float: right; margin: 0 0 2em 2em;"></a>
...
<a href="https://readr.tidyverse.org/" title="readr website"><img src="images/readr.png" style="width: 33%; max-width: 150px; float: right; margin: 0 0 2em 2em;"></a>
...
packages loaded by the `tidyverse` package, so
all we need to do to use this package is include the code `library(tidyverse)`
on the first line of our R script. We will use comments (lines of code beginning 
with `#`) to help explain as we go.

```r
# load the tidyverse suite of packages, including the readr package that 
# contains the read_csv() function
library(tidyverse)
...
the keyboard shortcut `Option+-` (on Mac) or `Alt+-` (on Windows or Linux) to 
...
Use `read_csv()` to load data from `https://github.com/mpjashby/crimemapping/raw/main/inst/extdata/san_francisco_robbery.csv` and store it in an object called `san_fran_rob`. Use the code above to help you, or click the
...
is somewhat esoteric, so we can ignore the details for now.

To see the data currently stored in the `san_fran_rob` object, we can just type
the object name on a line of code on its own and click the `Run Code` button.
...
<a href="https://readxl.tidyverse.org/" title="readxl website"><img src="images/readxl.png" style="width: 33%; max-width: 150px; float: right; margin: 0 0 2em 2em;"></a>
...
computer. To get round this file, we will first download an Excel file and store
it in a temporary directory (to avoid cluttering up our computers).

```{r read-exercise3, exercise=TRUE}
...
We can now load the sheet containing data for Austin:
...
quiz(
  ...
  question(
    ...
    answer(
      "`san_fran_rob` (i.e. just the name of the object)",
      correct = TRUE,
      message = "To see the first few rows of a tibble in R, just type its name"
    ),
    answer(
...
```{r dplyr-image, fig.align="center", out.width="80%"}
...
<a href="https://dplyr.tidyverse.org/" title="dplyr website"><img src="images/dplyr.png" style="width: 33%; max-width: 150px; float: right; margin: 0 0 2em 2em;"></a>
...
instead produce (or *return*) new ones. This means we have to assign the value
returned by each function to a new object (or overwrite the existing object):

```{r select-exercise5, exercise=TRUE}
agg_assault_locations <- select(agg_assault_data, lon = longitude, lat = latitude)

agg_assault_locations
```
...
```{r filter-image, fig.align="center", out.width="80%"}
...
Note that: 
  ...
  * the column name `location_type` is not surrounded by quotes but the column 
    value `"residence"` is, and
  * the `==` (equal to) operator is used, since a single equals sign `=` has 
    another meaning in R (but shouldn't be used).
...
quiz(
  ...
  question(
    'Which offences (rows) will be returned by the code `filter(agg_assault_data, location_type == "restaurant" | location_type == "mall")`?',
...
```{r mutate-image, fig.align="center", out.width="80%"}
knitr::include_graphics("images/mutate.jpg")
```
...
mutate(
  agg_assault_data, 
  region = ifelse(latitude > median(latitude), "northern", "southern")
)
```
...
mutate(
  ...
  # value) using the ifelse() function
  location_type = ifelse(location_type == "street", "road", location_type),
...
<a href="https://lubridate.tidyverse.org/" title="lubridate website"><img src="images/lubridate.png" style="width: 33%; max-width: 150px; float: right; margin: 0 0 2em 2em;"></a>
...
summarised using a function that you specify. For example, if you wanted to
identify the weighted centre of the locations of all the aggravated assaults in
our dataset (perhaps to decide at which police station to base a specialist
squad):
...
The `na.rm = TRUE` argument specifies that the `mean()` function should remove
(`rm`) and missing (`NA`) values before calculating the mean. If we do not 
specify this and our data contain any missing values, the `mean()` function will 
return `NA`. We could simplify the above code slightly using the `across()`
function, but we do not need to learn about that now. You can [learn about the
`across()` function in this tutorial](https://dplyr.tidyverse.org/articles/colwise.html).
...
groups have been handled by `summarise()`. This warning reminds you that
`summarise()` strips the final level of grouping added by `group_by()`. We don't
need to go into the reason for this, but if you want to hide the message then
you can add `.groups = "drop"` to the `summarise()` function to remove all the
groups in the data.
...
data1 <- read_csv("https://github.com/mpjashby/crimemapping/raw/main/inst/extdata/san_francisco_robbery.csv")

# select all except the offense_type column, since it has the same value for
# every row
data2 <- select(data1, -offense_type)

# filter only those offences that occurred in January 2019
data3 <- filter(data2, as.Date(date_time) < as.Date("2019-02-01"))

# create a new weekday variable
data4 <- mutate(data3, weekday = wday(date_time, label = TRUE))

# group offences by weekday
data5 <- group_by(data4, weekday)

# count how many offences occurred on each weekday
jan_weekday_counts <- count(data5)
...
expected by `select()`, `filter()`, `mutate()`, `group_by()` and `count()` is
...
jan_weekday_counts <- count(group_by(mutate(filter(select(
  read_csv("https://github.com/mpjashby/crimemapping/raw/main/inst/extdata/san_francisco_robbery.csv"),
  -offense_type), as.Date(date_time) < as.Date("2019-02-01")), 
  weekday = wday(date_time, label = TRUE)), weekday))
...
example, it's quite hard (without counting pairs of brackets) to work out that
the reference to the column `weekday` at the end of the fourth line of code
belongs to the `group_by()` function on the first line.

<a href="https://magrittr.tidyverse.org/" title="magrittr website"><img src="images/magrittr.png" style="width: 33%; max-width: 150px; float: right; margin: 0 0 2em 2em;"></a>
...
code. This method uses the `%>%` (pipe) operator from the
[`magrittr`](https://magrittr.tidyverse.org/#usage) package. The pipe operator
works by using the result of the code on the left-hand side of the pipe as the
first argument to the function on the right-hand side. So the code `x %>% fun1()
...
can just omit the first argument to each function after the first one.

```{r pipe-exercise3, exercise=TRUE}
jan_weekday_counts <- read_csv("https://github.com/mpjashby/crimemapping/raw/main/inst/extdata/san_francisco_robbery.csv") %>% 
  select(-offense_type) %>% 
  filter(as.Date(date_time) < as.Date("2019-02-01")) %>% 
...
examples in future tutorials. In the meantime, you can find out more about how 
to use the pipe operator in the [Introducing `magrittr`](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html) online tutorial.


## In summary

In this tutorial, we have learned how to wrangle data in R using functions from
packages in the `tidyverse` suite of packages. We can now construct a complete
...
further, you might be interested in:
  ...
  * [Data Wrangling with dplyr and tidyr cheat sheet](https://rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf) 
    by the team at RStudio, which provides a handy two-page guide to the main
    functions in the `dplyr` package, which is very useful for reminding you of
    the code needed to run each of the functions we have used in this tutorial.
...
```{r conclusion-image, fig.align="center", out.width="80%"}

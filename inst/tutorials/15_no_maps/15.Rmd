...
knowledge of the area and so an inset map such as this would not be needed.
...
Bar charts are useful for showing values of a continuous variable (e.g. a
...
anchor point. If we have positioned the legend in a corner of the plot, this is
...
`legend.justification` to the same value as you used for `legend.position`; if
you want the legend to grow 'outwards', just set `legend.justification` to the 
opposite (e.g. 
`theme(legend.justification = c(0, 1), legend.position = c(1, 0))`). In this
case, we want the legend to be anchored in the bottom-right corner and to grow 
inwards from it, so we will set both arguments to `c(1, 0)`.
...
<h5 id="dist-box1-title" class="box-title">Why did we set `axis.text.y = element_blank()`?</h5>

<div id="dist-box1" class="box-content">

By default, `geom_dotplot()` produces a vertical (y) axis labels that suggest
the y axis shows the proportion of points in each stack. However, these
proportions are only true in certain circumstances, so they can be misleading.
Since it's obvious what the vertical axis represents, it's better to remove the
potentially misleading axis text in this case.
...
<script>
$("#dist-box1-title").click(function () { $("#section-dist-box1").toggle("slow") })
</script>
...
burglary %>% 
  filter(district == "Kettering") %>% 
  ggplot() +
  geom_dotplot(aes(x = count), binwidth = 1, colour = NA, fill = "grey50") +
...
burglary %>%
  filter(district == "Kettering") %>%
  ggplot(aes(x = count)) +
  geom_dotplot(binwidth = 1, colour = NA, fill = "grey50") +
...
make it clearer:
  ...
  * Removed the grid lines for the *y* axis by setting the `theme()` attributes
    `panel.grid.major.y` and `panel.grid.minor.y` to `element_blank()`.

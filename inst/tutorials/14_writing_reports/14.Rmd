...
`Rmarkdown` indicate that it should be emphasised (typically with *italic* 
text).
...
Since Rmarkdown can produce many types of document, RStudio has a dialogue box in
...
<p class="centered-image"><img src="images/rstudio-markdown-menu.png" alt="Screen shot of RStudio's dialogue box for creating a new Rmarkdown document."></p>
...
added by packages that you have installed. For example, you can create a resume
in Rmarkdown with the [`vitae` package](https://pkg.mitchelloharawild.com/vitae/),
so if that packages is installed on your computer then you will see a resume
template as one of the options available in this dialogue box..
...
<p class="centered-image"><img src="images/rstudio-new-markdown.png" alt="Screen shot of RStudio's dialogue box for creating a new Rmarkdown document."></p>
...
include the characters ```` ``` ```` on a line on their own. There must not be
any spaces or other characters on the line before ```` ```{r} ```` or 
```` ``` ````.
...
_output_ produced by the code in each code chunk (e.g. and messages or warnings) 
should not be shown, we can set the chunk option `include=FALSE`. So if we 
wanted our code chunk to produce no output at all, we could change the first 
line of the code chunk to read
...
`echo = FALSE`. We can also use the `setup` chunk to load any packages we need 
...
you can't see because you've set `warning=FALSE`. So if you set either of these 
options, make sure you take them out again if you run into any problems. Even 
better, only set these options when you've already run your complete document 
...
**I recommend you print out or keep a copy of this guide so that you can refer
to it while you are writing Rmarkdown documents**.
...
`tmap`) that Word and PDF format cannot, and can easily be read on mobile 
...
It also means we can produce periodic
